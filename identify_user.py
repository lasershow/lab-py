    ...
    for i in list(crash_index):
        ...
        for count in range(1, MAX_CHECK_COUNT):
            ...
            if crash_host == d[i+(-1)*count]:
                ...
                if is_username(log_df.iloc[i+count, ].Detail):
                    username_index = i+count
                    print("==========")
                    print(count)
...
def main():
    ...
    log_df = log_df.rename(columns={'index': 'Level',
                                    'Level': 'Date and Time',
                                    'Date and Time': 'Source',
                                    'Source': 'Event ID',
                                    'Event ID': 'Task Category',
                                    'Task Category': 'Log',
                                    'Log': 'Computer', 'Computer': 'Detail'})

    # log_df.rename(columns={'Unnamed: 7': 'Detail'}, inplace=True)
    log_df = create_crash_user_df(log_df)
    log_df.to_csv('./crash_log_'+basename+'.csv')
